...
void SDL_SaveImage(screen *s, const char *filename) {
  ...
  SDL_Surface *surf =
      SDL_CreateRGBSurfaceFrom((void *)s->buffer, s->width, s->height, 32,
                               s->width * sizeof(uint32_t), rmask, gmask, bmask, amask);
...
screen *InitializeSDL(int width, int height, bool fullscreen) {
  ...
  SDL_VERSION(&compiled);
  if (compiled.major < 2) {
    std::cout << "Could not initialise SDL: Requires SDL2 headers (current " << (int)compiled.major
              << "." << (int)compiled.minor << ")" << std::endl;
    exit(1);
  }
  ...
  if (linked.major < 2) {
    std::cout << "Could not initialise SDL: Requires SDL2 runtime (current " << (int)linked.major
              << "." << (int)linked.minor << ")" << std::endl;
    exit(1);
  }
  ...
  s->window = SDL_CreateWindow("COMS30115", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width,
                               height, flags);
  if (s->window == 0) {
  ...
  s->texture = SDL_CreateTexture(s->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC,
                                 s->width, s->height);
  if (s->texture == 0) {
